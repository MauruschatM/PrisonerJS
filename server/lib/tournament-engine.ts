...
function executeStrategy(
	...
	try {
      ...
      `
		...
		const result = strategyWrapper(safeContext, opponentHistory, myHistory, round);
...
export async function playGame(
	...
	for (let round = 0; round < rounds; round++) {
		try {
			...
			const player1Move = executeStrategy(strategy1Code, player2History, player1History, round);
			const player2Move = executeStrategy(strategy2Code, player1History, player2History, round);

			// Calculate scores for this round
			const moveKey = `${player1Move}${player2Move}` as keyof typeof PAYOFF_MATRIX;
				...
				if (participant1.length > 0) {
					...
					await db
						...
						.set({
							...
							wins: gameResult.winner === "player1" ? (p1.wins || 0) + 1 : p1.wins || 0,
							losses: gameResult.winner === "player2" ? (p1.losses || 0) + 1 : p1.losses || 0,
							draws: gameResult.winner === "draw" ? (p1.draws || 0) + 1 : p1.draws || 0,
						})
				...
				if (participant2.length > 0) {
					...
					await db
						...
						.set({
							...
							wins: gameResult.winner === "player2" ? (p2.wins || 0) + 1 : p2.wins || 0,
							losses: gameResult.winner === "player1" ? (p2.losses || 0) + 1 : p2.losses || 0,
							draws: gameResult.winner === "draw" ? (p2.draws || 0) + 1 : p2.draws || 0,
						})
