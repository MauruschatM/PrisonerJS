...
export async function createWeeklyTournament(): Promise<string> {
  try {
    ...
    const newTournament = {
      ...
      name: tournamentName,
      description:
        "Automatisches wöchentliches Tournament - jeden Samstag um 20:00 Uhr",
      roundsPerMatch: 200,
      scheduledAt: now,
...
export function startTournamentScheduler(): void {
  ...
  const scheduleNext = () => {
    ...
    console.log(
      `Next tournament scheduled for: ${nextRun.toLocaleString("de-DE")}`
    );
    console.log(
...
export function startDevelopmentScheduler(): void {
  console.log(
    "Starting development tournament scheduler (1 minute intervals)..."
  );
  ...
  const scheduleNext = () => {
    ...
    console.log(
      `Next development tournament in: ${Math.round(timeUntilNext / 1000)} seconds`
    );
